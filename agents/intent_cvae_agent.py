...
class DiscreteIntentCopyGRUAgent(TorchGeneratorAgent):
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        if shared is None:
            ...
            self.intent_keywords = [intent_1gram, intent_2gram, intent_3gram]
            ...
            self.ours_output = open('./new_output.txt', 'w')
    ...
    def batchify(self, obs_batch, sort=False):
        ...
        intent_1gram = self.intent_keywords[0]
        ...
        if self.model.training is True:
            if np.random.rand() < (1 - self.exposure):
                scores, preds, copy_rates = model.decode_forced(emotion_pred, \
                    posterior_intent, self.intent_keywords[0], encoder_states, batch.label_vec)
            else:
                prior_intent = gumbel_softmax_sample(prior_intent_logit).argmax(dim=-1)
                scores, preds, copy_rates = model.decode_forced(emotion_pred, \
                    prior_intent, self.intent_keywords[0], encoder_states, batch.label_vec)
    ...
    def greedy_generate(self, batch):
        ...
        # exs = batch.observations[0]
        # emoion = 'Emotion: ' + exs['emotion'] + '\n' 
        # context = 'Context: ' + exs['full_text'].lower().replace('\n', ' [SEP] ') + '\n'
        # reference = 'Reference: ' + self.translate(batch.labels[0]) + '\n'
        ...
        #     self.ours_output.write("Intent: %s, Probability: %f \n" % (intent, prior_prob[0][idx].item()))
        #     pred = self.model.decode_greedy \
        #         (emotion_pred, torch.tensor([idx]).cuda(), self.intent_keywords, encoder_states, 32)
            
        #     text = self._v2t(pred[0])
        ...
        #   (emotion_pred, posterior_intent, self.intent_keywords, encoder_states, 32)
        preds = self.model.decode_greedy \
            (emotion_pred, prior_intent, self.intent_keywords[0], encoder_states, 32)
    ...
    def beam_generate(self, batch):
        ...
        encoder_states = self.model.encoder(*self._encoder_input(batch))
        sample_preds = beam_search(encoder_states,
                            model=self.model,
                            start_idx=self.dict[self.dict.start_token],
                            end_idx=self.dict[self.dict.end_token],
    ...
    def eval_step(self, batch):
        ...
        for i, exs in enumerate(batch.observations):
            ...
            # for idx, intent in self.intents: 
    ...
    def _add_bleu_metrics_for_samples(self, batch):
        ...
        prior_prob = (1-self.prior_rate) * F.softmax(prior_intent_logit, dim=-1) \
                        + self.prior_rate * emotion_pred.matmul(self.intent_prior)
        prior_intent = gumbel_softmax_sample(torch.log(prior_prob)).argmax(dim=-1)
        # prior_intent = gumbel_softmax_sample(torch.zeros_like(prior_prob).to(prior_prob.device)).argmax(dim=-1)


        # preds = self.model.decode_greedy \
        #   (emotion_pred, posterior_intent, self.intent_keywords, encoder_states)
        preds = self.model.decode_greedy \
            (emotion_pred, prior_intent, self.intent_keywords[0], encoder_states)
