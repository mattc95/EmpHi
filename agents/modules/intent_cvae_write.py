...
class EmotionIntentDecoder(nn.Module):
    ...
    def __init__(self, implicit, implicit_dynamic, explicit, num_layers, embeddings, hidden_size, padding_idx):
        """
        Initialization.
        Arguments here can be used to provide hyperparameters.
        """
        ...
        self.intent_read_gate = nn.Sequential(
            nn.Linear((3+self.num_layers)*hidden_size, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
            nn.Sigmoid(),
        )
        
        self.intent_write_gate = nn.Sequential(
            nn.Linear((1+self.num_layers)*hidden_size, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
            nn.Sigmoid(),
        )
    ...
    def forward(self, seq, emotion_emb, intent_state, context, state, mask):

        bsz = seq.size(0)
        seq_emb = self.embeddings(seq)
        s = state.permute(1, 0, 2).contiguous().view(bsz, -1)
        ss = state.permute(1, 0, 2).contiguous().mean(dim=-2).unsqueeze(dim=-1)

        """
            context attention
        """
        context_attention = context.matmul(ss).squeeze(dim=-1)
        ...
        if self.implicit is True:
            if self.implicit_dynamic is True:
                ...
                read_input = torch.cat([seq_emb, context_value, s, emotion_emb], dim=-1)
                emotion_attention = self.emotion_read_gate(read_input)
                emotion_read = emotion_attention * emotion_emb

                read_input = torch.cat([seq_emb, context_value, s, intent_state], dim=-1)
                intent_attention = self.intent_read_gate(read_input)
                intent_read = intent_attention * intent_state

                input = torch.cat([input, emotion_read, intent_read], dim=-1)
            else:
                input = torch.cat([input, emotion_emb, intent_state], dim=-1)
        ...
        if self.implicit_dynamic:
            s = state.permute(1, 0, 2).contiguous().view(bsz, -1)
            write_input = torch.cat([intent_state, s], dim=-1)
            intent_attention = self.intent_write_gate(write_input)
            intent_state = intent_attention * intent_state           

        return output, state, intent_state
...
class IntentCVAEGRUModel(GRUModel):
    ...
    def __init__(self, opt, dictionary):
        ...
        self.decoder = EmotionIntentDecoder \
            (opt['implicit'], opt['implicit_dynamic'], opt['explicit'], self.num_layers, self.embeddings, hidden_size, dictionary[dictionary.null_token])
    ...
    def decode_forced(self, emotion, intent, intent_knowledge, encoder_state, ys, ys_lens):
        ...
        intent_state = torch.zeros(bsz, res_len+1, self.hidden_size).float().to(device)
        ...
        intent_1gram, _, _ = intent_knowledge
        ...
        cur_intent_state = intent_emb

        for t in range(res_len):

            out, state, cur_intent_state = \
            self.decoder(inputs, emotion_emb, cur_intent_state, context, state, mask)

            intent_state[:, t+1] = cur_intent_state
        ...
        end_state = intent_state[batch_idx, ys_lens]

        return logit, pred, copy_rate, end_state
    ...
    def decode_greedy(self, emotion, intent, intent_knowledge, encoder_states, max_len=64):
        ...
        intent_state = intent_emb

        for t in range(max_len):

            out, state, intent_state = \
                self.decoder(inputs, emotion_emb, intent_state, context, state, mask)
