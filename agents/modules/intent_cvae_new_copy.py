...
class IntentCVAEGRUModel(GRUModel):
    ...
    def output(self, output, state, intent_emb, intent_index, intent_knowledge):
        """
        ...
        copy_rate = F.sigmoid((state[-1] * intent_emb).sum(dim=-1, keepdim=True))
    ...
    def decode_forced(self, emotion, intent, intent_knowledge, encoder_state, ys):
        ...
        for t in range(res_len):
            ...
            logit[:, t], copy_rate[:, t] = self.output(out.squeeze(dim=1), state, intent_emb, intent_index, intent_knowledge)
    ...
    def decode_greedy(self, emotion, intent, intent_knowledge, encoder_states, max_len=64):
        ...
        for t in range(max_len):
            ...
            logit, _ = self.output(out.squeeze(dim=1), state, intent_emb, intent_index, intent_knowledge)
            # Place predictions in a tensor holding predictions for each token
