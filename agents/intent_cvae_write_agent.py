...
from .modules.write_copy_model import IntentCVAEGRUModel
...
class DiscreteIntentCopyGRUAgent(TorchGeneratorAgent):
    ...
    def compute_loss(self, batch, return_output=False):
        ...
        state_input = state.permute(1, 0, 2).contiguous().view(batchsize, -1)
        prior_intent_logit = self.model.intent_prediction(state_input)
        ...
        emotion_logit = self.model.emotion_prediction(state_input)
        ...
        if self.model.training is True:
            scores, preds, copy_rates, end_state = model.decode_forced(emotion_pred, \
                posterior_intent_prob.argmax(dim=-1), self.intent_knowledge, encoder_states, batch.label_vec, batch.label_lengths)
        else:
            scores, preds, copy_rates, end_state = model.decode_forced(emotion_pred, \
                posterior_intent_prob.argmax(dim=-1), self.intent_knowledge, encoder_states, batch.label_vec, batch.label_lengths)
        ...
        if self.implicit_dynamic is True:
            l2 = LA.norm(end_state, dim=-1)
            
            self.record_local_metric('l2', AverageMetric.many(l2))
            total_loss = total_loss + self.theta * l2
            loss = loss + self.theta * l2.mean()
    ...
    def greedy_generate(self, batch):
        ...
        preds = self.model.decode_greedy \
          (emotion_pred, posterior_intent, self.intent_knowledge, encoder_states, 24)
        # preds = self.model.decode_greedy \
        #     (emotion_pred, prior_intent, self.intent_knowledge, encoder_states, 24)
    ...
    def _add_bleu_metrics_for_samples(self, batch):
        ...
        """
            Intent prediction
        """
        state_input = state.permute(1, 0, 2).contiguous().view(batchsize*sample_num, -1)
        prior_intent_logit = self.model.intent_prediction(state_input)
        ...
        for k in range(1, 5):
            self.record_local_metric(f'precision_bleu_{k}', precision_bleu[k])
            self.record_local_metric(f'recall_bleu_{k}', recall_bleu[k])
